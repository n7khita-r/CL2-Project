"""
Naive Bayes Genre Classification with Independent Feature Groups
...
# ============================================================================
def load_data(data_dir='./data', samples_per_genre=3000):
    """Load CSV files with balanced sampling per genre."""
    all_data = []
    
    if not os.path.exists(data_dir):
        print(f"Error: Data directory '{data_dir}' not found!")
        print("Please ensure your data directory exists and contains genre CSV files.")
        sys.exit(1)
    
    csv_files = [f for f in os.listdir(data_dir) if f.endswith('.csv')]
    genre_files = [f for f in csv_files if f not in ['movies_processed.csv']]
    
    if not genre_files:
        print(f"Error: No genre CSV files found in '{data_dir}'")
        sys.exit(1)
    
    print(f"Found {len(genre_files)} genre CSV files")
    print(f"Target: {samples_per_genre} samples per genre\n")
    
    for csv_file in sorted(genre_files):
        ...
        try:
            ...
            if 'description' in df.columns:
                ...
                if len(df_subset) > samples_per_genre:
                    df_subset = df_subset.sample(n=samples_per_genre, random_state=42)
                    print(f"✓ Sampled {samples_per_genre} from {csv_file} (had {len(df)})")
                else:
                    print(f"✓ Using all {len(df_subset)} samples from {csv_file} (limited)")
                
                all_data.append(df_subset)
            else:
                print(f"✗ 'description' column not found in {csv_file}")
...
# ============================================================================
def plot_confusion_matrix(y_true, y_pred, classes, save_path='results/naivebayes/noaffixesconfusion_matrix.png'):
...
def evaluate_classifier(y_true, y_pred, classes, save_path='results/naivebayes/noaffixesevaluation_metrics.txt'):
...
def main():
    print("\n" + "="*80)
    print("NAIVE BAYES GENRE CLASSIFICATION")
    ...
    print("\n7. Top features per genre (by log probability):")
    top_features = classifier.get_top_features(n=10)
    
    features_output = []
    features_output.append("="*80)
    features_output.append("TOP FEATURES PER GENRE (by log probability)")
    features_output.append("="*80)
    
    for genre in sorted(top_features.keys()):
        print(f"\n{genre.upper()}:")
        features_output.append(f"\n{genre.upper()}:")
        for feature, log_prob in top_features[genre][:5]:
            line = f"  {feature:30s} {log_prob:+.4f}"
            print(line)
            features_output.append(line)
    
    features_path = 'results/naivebayes/noaffixestop_features.txt'
    ...
    predictions_path = 'results/naivebayes/noaffixespredictions.csv'
    predictions_df.to_csv(predictions_path, index=False)
    print(f"✓ Predictions saved to: {predictions_path}")
    
    summary_path = 'results/naivebayes/noaffixessummary.txt'
    ...
    print("="*80)
    print(f"\n✓ All results saved to results/naivebayes/noaffixes")
