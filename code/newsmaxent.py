"""
Maximum Entropy category Classification with Linguistically-Motivated Features
...
packages = ['pandas', 'numpy', 'scikit-learn', 'matplotlib', 'seaborn', 'nltk', 'spacy', 'scipy', 'json']
for pkg in packages:
    install_package(pkg)
...
class MaxEntTextClassifier:
    ...
    def __init__(self, C=2.0, max_iter=500):
        self.C = C
...
# ============================================================================
def plot_confusion_matrix(y_true, y_pred, classes, save_path='../results/MaxEnt/News/CCONFUSIONNEWSconfusion_matrix.png'):
...
def evaluate_classifier(y_true, y_pred, classes, save_path='../results/MaxEnt/News/evaluation_metrics.txt'):
...
def main():
    print("\n" + "="*80)
    print("MAXIMUM ENTROPY category CLASSIFICATION")
    ...
    for category, count in train_dist.items():
        print(f"  {category:15s}: {count:4d} samples")
    ...
    print("\n7. Top features per category (by absolute weight):")
    top_features = classifier.get_top_features(n=10)
    
    features_output = []
    features_output.append("="*80)
    features_output.append("TOP FEATURES PER category (by absolute weight)")
    features_output.append("="*80)
    ...
    features_path = '../results/MaxEnt/News/top_features.txt'
    ...
    predictions_path = '../results/MaxEnt/News/predictions.csv'
    predictions_df.to_csv(predictions_path, index=False)
    print(f"✓ Predictions saved to: {predictions_path}")
    
    summary_path = '../results/MaxEnt/News/summary.txt'
    ...
    print("="*80)
    print(f"\n✓ All results saved to ../results/MaxEnt/News/")
